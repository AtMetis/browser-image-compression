...
export default async function compress(file, options, previousProgress = 0) {
  ...
  if (!shouldReduceResolution) {
    ...
    let low = 0; // lowes possible quality
    let high = 1; // higest possible quality

    while (low <= high && remainingTrials-- && (currentSize > maxSizeByte || currentSize > sourceSize)) {
      ...
      // eslint-disable-next-line no-await-in-loop
      compressedFile = await canvasToFile(canvas, outputFileType, file.name, file.lastModified, quality);
      currentSize = compressedFile.size;

      // If true, adjust the lower bound to narrow the search quality.
      if (currentSize < maxSizeByte) low = mid;
      else high = mid - 0.001; // else adjust the upper bound to narrow the search quality.
